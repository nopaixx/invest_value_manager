# Sector: Technology / Software

> Ultima actualizacion: 2026-04-07 (incremental refresh: oil $111 WTI, ALFA.L FY2025 results, AI model updates, Liberation Day anniversary, ETF levels updated)
> Analista: Claude
> Status: **NEUTRAL** (selectivo en quality compounders, evitar growth sin fundamentales)
>
> **April 7 update (incremental):** (1) Oil surged to $111.50 WTI (T1 verified, was $94 in prior view) -- Hormuz still closed, EIA expects Brent peak $115/b in Q2 2026. Pure software has NO direct oil sensitivity but recession/stagflation risk elevated, pushing rate cuts FURTHER out. (2) ALFA.L reported FY2025: revenue GBP 126.7M (+17% CC), 32% op margin, ARR GBP 43.9M (+15%), NRR 109%, 10 late-stage prospects. New CFO Andrew Dickson (from Cerillion) joining Jul 6 as CFO designate, exec director Sep 16 -- Duncan Magrath retiring. Ex-div Apr 30. (3) Liberation Day tariffs 1-year anniversary Apr 2 -- software sector confirmed IMMUNE to direct tariff impact. Section 301 probes ongoing but pure software unaffected. IEEPA ruling still holds. (4) AI acceleration: Claude Opus 4.6 + GPT-5.4 live, MCP crossed 97M installs (Mar 2026), Microsoft Council runs multiple AI models simultaneously. Agentic AI Foundation formed under Linux Foundation. GPT-6 expected May-Jul, Claude 5 "Fennec" May-Sep. Enterprise agentic AI entering production phase -- seat compression narrative intensifying. (5) ADBE at $240.14 (P/E 14.0x, near 52wL $233.16), consensus PT ~$344-366 (44-52% upside). CEO search ongoing. No CMA UK investigation found. (6) DOCS at $22.50 near 52wL $21.82. Q4 FY2026 earnings May 19. NRR 112%. 300K+ prescribers using AI products. (7) GDDY at $83.28, earnings Apr 30. (8) IGV $80.50 (-5% from prior $85), XLK $137.43 (-1.5% from $139.54). Software underperforming energy by 50pp+ YTD. VIX 25.78 (elevated but down from 57.96 peak).
>
> **S155b Context (Mar 30):** AI "Reckoning" confirmed -- MSCI Software Index -21% YTD, per-seat model under structural agentic AI threat. BUT sector Q1 EPS expected +45.1% (best of all 11 sectors). ADBE upped FY2026 guidance post-Q1 BEAT. GDDY FY2026 ~6% growth confirmed. ALFA.L new CFO appointed Mar 2. Supreme Court ruled IEEPA tariffs unlawful -- reduces trade war escalation risk for software. Oil still ~$94 WTI (Iran war ongoing). Rate cuts still Oct-Dec 2026.
> **PRIOR (Mar 18):** Fed holds 3.50-3.75%. ADBE -7% on CEO transition announcement. ADBE Q1 BEAT $6.4B / EPS $6.06. AI-first ARR tripled YoY. FTNT EXIT approved late April.
> CRISIS NOTE (ESCALATING): Iran war Day 38+, Hormuz closed, oil $111.50 WTI (T1). Software selling on GENERAL risk-off, NOT sector-specific fears. Enterprise IT spending growing 10-15% (Gartner $6.15T forecast) but stagflation risk is REAL at $111+ oil. Rate cuts now likely pushed to Q1 2027 if oil stays above $100.

## Resumen Ejecutivo

El sector Technology/Software sigue siendo el mercado mas grande y dinamico del mundo, con Gartner pronosticando gasto IT global de $6.15T en 2026 (+10.8% YoY), y software enterprise creciendo ~14.7% impulsado por adopcion de AI y price increases por funcionalidades generativas. Gartner estima gasto total en AI de $2.52T en 2026 (+44% YoY), con GenAI model spending creciendo 80.8%. Sin embargo, el mercado esta atravesando TRES presiones simultaneas:

1. **SaaSpocalypse estructural (2025-2026):** IGV (software ETF) trading at ~$80.50, down further from $85. OIH (oil services) has crushed IGV by 50pp+ YTD -- most violent sector rotation in years. El mercado reprecia el riesgo de que AI commoditice software seat-based. AI-native companies crecen al DOBLE que SaaS tradicional, comprimiendo ventanas competitivas. Pero Copilot de Microsoft tiene solo 15M asientos (3.3% de la base de 450M de M365 despues de 2 anos), y pierde share vs ChatGPT. Esto sugiere que AI disruption es real pero NO esta fluyendo a traves de los incumbentes bundled copilots como se temia.

2. **Iran war risk-off + oil shock (week 5+, Apr 2026):** Oil now $111.50 WTI (T1 verified), up from $94 in prior view. EIA forecasts Brent peaking at $115/b in Q2 2026. Software vendiendo por risk-off general, no por miedo sectorial especifico. Oil at $111+ creates REAL stagflation risk that could delay rate cuts to Q1 2027. Fed FOMC held at 3.50-3.75%. Rate cuts now VERY unlikely before year-end if oil stays elevated. This is CYCLICAL, not STRUCTURAL for the sector -- but the cyclical pain is intensifying.

3. **Trade war / Section 301 (1-year anniversary):** Liberation Day tariffs turned 1 year old on Apr 2, 2026. Software confirmed IMMUNE to direct tariff impact (digital goods, no physical supply chain). Section 301 probes into 16+ economies ongoing. Supreme Court IEEPA ruling still holds -- limits further tariff escalation. Manufacturing employment -89K since Apr 2025 but software employment unaffected. Indirect impact through customer IT budget uncertainty persists but is more manageable without escalation risk.

**Mi tesis sobre el sector (ACTUALIZADA Apr 7):** Tesis MANTENIDA. El oil shock se ha agravado ($94 -> $111.50) pero el impacto directo en software sigue siendo NULO -- las 3 posiciones no tienen exposicion a commodities ni supply chain fisica. El riesgo INDIRECTO ha aumentado: (1) mayor probabilidad de recesion/stagflation -> potencial recorte de IT budgets, (2) rate cuts pushed further out -> extended multiple compression for growth stocks. Sin embargo: las empresas que tenemos son de pricing power excepcional (ADBE 89% gross margin, GDDY >33% EBITDA margin, ALFA.L 32% op margin) y sus clientes (creative professionals, SMBs, asset finance firms) no estan directamente afectados por oil prices. El "AI Reckoning" continua siendo la amenaza ESTRUCTURAL mas importante, pero nuestras posiciones estan PARCIALMENTE AISLADAS por modelos non-seat o outcome-based. El panorama de AI se esta acelerando con MCP a 97M installs, agentic AI entering production, y GPT-6/Claude 5 esperados en los proximos 3 meses -- esto CONFIRMA que seat compression es real pero tambien que incumbentes con data moats (como ADBE) se benefician de la wave.

**NOTA CRITICA (Mar 18):** ADBE CEO Narayen stepping down. Q1 FY2026 BEAT ($6.4B vs $6.28B est, EPS $6.06 vs $5.87). AI-first ARR tripled. Firefly subs +75% QoQ. MAU >850M (+17% YoY). FY2026 guidance upped: revenue $25.9-26.1B, non-GAAP EPS $23.30-23.50. Market focused on CEO uncertainty, ignoring operational strength. New KC#9 added: successor fails to maintain AI/growth strategy + rev growth <8% for 2 consecutive quarters. FTNT EXIT approved -- selling late April.

**NEW (Mar 30) -- AI RECKONING STRUCTURAL UPDATE:** Enterprises reporting "seat compression" -- single AI agents replacing dozens of human software licenses. Microsoft stock -21% YTD. VC flowing OUT of public software, INTO AI-native startups. This is NOT just multiple compression -- it represents a genuine business model challenge for seat-based SaaS. Assessment for our positions: ADBE (outcome-based + creative tools = partially insulated), GDDY (SMB web infrastructure = non-seat model), ALFA.L (asset finance enterprise = sticky long-term contracts). Risk is HIGHEST for generic workflow/analytics SaaS -- none of our positions.

**NEW (Mar 30) -- IEEPA TARIFF RULING:** Supreme Court ruled Trump's IEEPA tariff authority unlawful. Reduces tail risk of additional digital services tariffs. The WTO moratorium on e-transmission tariffs is also still active through mid-2026. Net: tariff risk to pure software remains LOW. Customer IT budget uncertainty persists but is more manageable without escalation risk.

**NEW (Apr 7) -- AGENTIC AI ACCELERATION:** MCP crossed 97M installs. Agentic AI Foundation formed under Linux Foundation (Dec 2025) with contributions from Anthropic MCP, OpenAI AGENTS.md, Block's goose. Microsoft Council runs OpenAI + Anthropic models simultaneously on same query. Claude Opus 4.6 leads agentic coding benchmarks. Claude Code adopted as preferred AI coding tool by developers. GPT-6 expected May-Jul 2026, Claude 5 "Fennec" May-Sep 2026. This CONFIRMS agentic AI is moving from experimental to production -- seat compression is not hypothetical, it is happening NOW. BUT: this also means AI-enhanced software (like ADBE Firefly) is MORE valuable, not less. The losers are commodity SaaS without AI differentiation.

---

## Metricas Clave

| Metrica | Valor | vs Historia | Tendencia |
|---------|-------|-------------|-----------|
| Global IT Spending | $6.15T (2026 Gartner) | +10.8% YoY | Acelerando por AI |
| Enterprise Software Spending | $1.43T+ (2026) | +14.7% YoY | Strong, AI-driven (slightly revised down from 15.2%) |
| Total AI Spending | $2.52T (2026 Gartner) | +44% YoY | Massive acceleration |
| TAM Global SaaS | $376B (2026) | +15.1% YoY | Creciendo 18.7% CAGR to 2034 |
| GenAI Model Spending | Growing 80.8% in 2026 | | AI infrastructure boom |
| AI Infrastructure | $1.37T (2026 Gartner) | >50% of total AI spend | Server spend +49% |
| Cloud Infrastructure Q4 2025 | $119.1B quarter | +30% YoY | AWS 24%, Azure 30%, GCP 32% |
| XLK (Tech ETF) | $137.43 (T1 Apr 7) | -9.4% from 52wH $153.00 | Weaker, oil drag |
| IGV (Software ETF) | $80.50 (T1 Apr 7) | -31.8% from 52wH $117.99 | Deep bear market, OIH crushing IGV by 50pp+ YTD |
| P/E sector (forward, XLK) | 32.7x (T1 Apr 7) | vs 25x (5y avg) | Compressing from selloff |
| Fed funds rate | 3.50-3.75% | HOLD | Next cut pushed to Q1 2027 if oil stays >$100 |
| Oil WTI | $111.50 (T1 Apr 7) | Was $94 in Mar 30 view | +18.6% in 8 days. Hormuz closed. Stagflation risk |
| VIX | 25.78 (T1 Apr 7) | Down from 57.96 peak | Elevated but improving |

**Fuentes:** [Gartner IT Spending $6.15T](https://www.gartner.com/en/newsroom/press-releases/2026-02-03-gartner-forecasts-worldwide-it-spending-to-grow-10-point-8-percent-in-2026-totaling-6-point-15-trillion-dollars), [Gartner AI Spending $2.52T](https://www.gartner.com/en/newsroom/press-releases/2026-1-15-gartner-says-worldwide-ai-spending-will-total-2-point-5-trillion-dollars-in-2026), [SaaStr: Software Spend +15.2%](https://www.saastr.com/gartner-enterprise-software-spend-will-grow-a-stunning-15-2-next-year-but-most-of-that-will-go-to-price-increases-and-ai-apps/), [macro_fragility.py T1 data Apr 7: WTI $111.50, XLK $137.43, VIX 25.78], [price_checker.py T1 data Apr 7: IGV $80.50, XLK $137.43]

---

## Estructura Competitiva

**Concentracion:** Oligopolio en nichos, fragmentado en conjunto
**Top 5 market share:** ~21% (SAP, Salesforce, Oracle, Microsoft, Intuit)

| Empresa | Nicho | Market Share | Fortaleza |
|---------|-------|--------------|-----------|
| Microsoft | SaaS general | #1 overall | Ecosistema, enterprise, Azure +30%. BUT Copilot only 3.3% penetration (15M/450M seats). NEW: Council feature runs multi-model AI (OpenAI + Anthropic simultaneously) |
| Salesforce | CRM | #1 (60%+) | Plataforma, integraciones. Agentforce pivot |
| Adobe | Creative/Docs | #1 (80%+ graphics) | Moat profesional, Firefly AI-first ARR tripled YoY. CEO TRANSITION underway. FTC SETTLED. |
| SAP | ERP | #1 tied | Enterprise lock-in, cloud transition |
| Oracle | Database/Cloud | #3 | Legacy + cloud transition |
| **Intuit** | **SMB Accounting + Tax** | **#1 (81% QBO, ~60% DIY tax)** | **GenOS platform, $100M OpenAI partnership** |
| Tyler Technologies | Gov Software | #1 (11.2% share) | Switching costs, 98% retention |
| **Dassault Systemes** | **PLM/3D Design** | **#1 (16.5% PLM)** | **90% aircraft, 80% autos. Cloud transition pain. NVIDIA partnership** |

**AI Adoption Reality Check (Updated Apr 7, 2026):**
- Microsoft Copilot: 15M seats = 3.3% of M365 base after 2 years. Market share FALLING (18.8% -> 11.5% of AI subscribers). Users prefer ChatGPT (76% vs 18% when both available)
- ChatGPT: 55.2% of paid AI subscribers. Enterprise plan growing. OpenAI valued at $852B (latest round)
- AI-native SaaS companies growing at 2x traditional SaaS rate (28-42% price premiums for AI features)
- Gartner: AI in "Trough of Disillusionment" throughout 2026 -- sold BY incumbent providers, not as moonshot projects. POSITIVE for established software companies with AI features
- Implication: AI features being adopted, but NOT necessarily through incumbent bundled copilots. Standalone AI tools winning user preference. This REDUCES the "Copilot bundles everything" disruption risk for point solution software (positive for ADBE)
- NEW (Apr 7): MCP (Model Context Protocol) at 97M installs -- becoming foundational agentic infrastructure. Claude Code emerging as preferred developer AI tool. Multi-model enterprise AI (Microsoft Council) is new paradigm -- reduces single-vendor lock-in risk
- NEW (Apr 7): GPT-6 expected May-Jul 2026, Claude 5 "Fennec" May-Sep 2026. Next generation of models will accelerate agentic capabilities further

**Fuentes:** [Recon Analytics: Copilot Adoption](https://www.reconanalytics.com/ai-choice-2026-why-licenses-dont-equal-adoption/), [Gartner AI Trough of Disillusionment](https://www.gartner.com/en/articles/strategic-predictions-for-2026), [SaaS Statistics 2026](https://www.hostinger.com/tutorials/saas-statistics), [Enterprise Agentic AI Landscape 2026](https://www.kai-waehner.de/blog/2026/04/06/enterprise-agentic-ai-landscape-2026-trust-flexibility-and-vendor-lock-in/)

---

## Ciclo y Sensibilidad

| Factor | Sensibilidad | Notas |
|--------|--------------|-------|
| Tipos de interes | **ALTA** | Tasas holding 3.50-3.75%. Rate cuts dead until Q1 2027 if oil stays >$100. Oil $111.50 supports higher-for-longer even more than before |
| Recesion | Media | Enterprise es sticky, SMB mas expuesto. Oil shock risk to IT budgets ELEVATED at $111+ WTI |
| Inflacion | Baja-Media | Pricing power para pasar costos. AI features enabling 15-25% price increases |
| Iran war / oil shock | **Media-Alta** (UPGRADED) | No direct impact on software. BUT $111 WTI creates real stagflation/recession risk that could compress IT budgets. Rate cut delay = extended multiple compression for growth stocks |
| Section 301 tariffs | **Baja** (direct) / **Media** (indirect) | Pure software = no tariff on product. Indirect: customer uncertainty, FX, semiconductor supply chain. 1-year post-Liberation Day: software confirmed immune |

**Mejor fase del ciclo:** Early/Mid (expansion de IT budgets)
**Beta tipico:** 1.2-1.5
**Correlacion con Fed:** Inversa - recortes de tasas = re-rating de software. Currently NEGATIVE: rate cuts off table, possibly until Q1 2027
**XLK from T1 (Apr 7):** $137.43 (-9.4% from 52wH, P/E 32.7x trailing)
**IGV from T1 (Apr 7):** $80.50 (-31.8% from 52wH $117.99, deep bear market)

---

## Disrupcion y Riesgos

### Tecnologicos (AI Impact) -- UPDATED Apr 7, 2026

| Amenaza | Probabilidad | Impacto | Timeline | Afectados |
|---------|--------------|---------|----------|-----------|
| AI commoditiza design | Media | Alto | 2-5 anos | Adobe, Canva |
| AI agents reemplazan software | Media-Alta (UPGRADED) | Muy Alto | 3-7 anos | CRM, Analytics. MCP at 97M installs confirms production readiness |
| AI coding assistants | Alta | Medio | Ya ocurriendo | IDEs, dev tools. Claude Code emerging as preferred tool |
| **AI bundles commoditizan FSS** | **Alta** | **Alto** | **Ya ocurriendo** | **Dropbox, Box** |
| **Copilot/ChatGPT replace point solutions** | **Media-Baja** (REVISED DOWN) | **Medio** | **3-5 anos** | **Less than feared. Copilot adoption disappointing. Microsoft now running multi-model (Council)** |
| **Agentic AI seat compression** | **ALTA (CONFIRMED, ACCELERATING)** | **Alto** | **Now-3 anos** | **Enterprises replacing dozens of seats with single AI agent. MCP 97M installs. Agentic AI Foundation formed. GPT-6/Claude 5 in 3 months will accelerate further. Per-seat SaaS model existentially challenged. ADBE/GDDY/ALFA.L have non-seat or outcome-based elements -- PARTIALLY INSULATED.** |
| AI enhances PLM/simulation | **Baja** | **Positivo** | 2-4 anos | **Dassault, Siemens (BENEFICIARIES)** |
| **AI-driven vendor consolidation** | **Alta** | **Medium** | **Now** | **Smaller SaaS vendors losing to platform plays** |
| **AI-native SaaS disruption** | **Media-Alta** | **Alto** | **1-3 anos** | **Traditional SaaS without AI features. AI-native growing 2x rate** |

**Mi view sobre AI disruption (UPDATED Apr 7):**
- The Copilot disappointment (3.3% penetration, losing share to ChatGPT) is BULLISH for incumbent point-solution software. Microsoft's Council multi-model feature is a PIVOT -- they're moving from Copilot-only to multi-model orchestration
- MCP at 97M installs and Agentic AI Foundation formed = agentic AI is entering PRODUCTION phase, not just experimentation. Seat compression is accelerating. But this helps incumbents WITH AI features (ADBE Firefly) and hurts those WITHOUT
- Enterprise AI spend is UP ($2.52T, +44% YoY) but flowing to FEWER vendors. Incumbents with data moats benefit: Adobe (Firefly trained on licensed content, AI-first ARR tripled), SAP (enterprise data), Intuit (financial data)
- Gartner says AI is in "Trough of Disillusionment" in 2026 -- enterprises buy AI from INCUMBENT providers, not moonshot startups. This is structurally positive for quality software companies
- The REAL losers: commodity SaaS without moats (FSS, generic analytics, simple workflow tools). These are being replaced by ChatGPT/Claude directly
- GenAI features enable 15-25% PRICING INCREASES for incumbents -- this is ACCRETIVE to revenue and margins for quality software companies

### Regulatorios

| Regulacion | Probabilidad | Impacto |
|------------|--------------|---------|
| EU Digital Markets Act | Ya vigente | Medio - afecta gatekeepers |
| US antitrust Big Tech | Media | Alto si se fragmentan |
| AI regulation | Alta | Incierto - puede beneficiar incumbentes |
| **Section 301 tariffs** | **Alta (probes ongoing, 1yr anniversary)** | **Bajo directo para software, medio indirecto via customer uncertainty** |
| **ADBE FTC** | **SETTLED ($150M)** | **RESOLVED. Trial CANCELLED. KC#6 removed.** |

### Competitivos

- **Nuevos entrantes:** Startups IA (OpenAI, Anthropic) amenazan verticalmente BUT losing enterprise to incumbents' own AI features. AI-native SaaS growing 2x rate is the real competitive pressure
- **Sustitutos:** Open source ganando share en infra (no tanto en apps)
- **Canva vs Adobe:** Canva AI threat real in prosumer, BUT Adobe's enterprise moat + Firefly differentiation holding. Firefly subs +75% QoQ validates monetization
- **Siemens Altair vs Dassault Simulia:** Consolidation in simulation creates stronger competition

---

## Sentimiento de Mercado

**Sentimiento actual:** **BEARISH, deepening.** IGV $80.50 (-31.8% from 52wH). XLK -9.4% from 52wH. Software underperforming energy by 50pp+ YTD -- most violent sector rotation in years. Oil at $111.50 intensifying risk-off. VIX 25.78 (elevated). Rate cuts pushed to Q1 2027.

**Narrativa dominante:**
"AI lo cambia todo. Solo invierte en los winners de AI (NVDA, MSFT). El resto es disruptable. Plus oil shock at $111 + tariffs = recession = cut IT budgets. Adobe losing its CEO at the worst time. Software is dead money until rates come down."

**Mi contra-tesis:**
"El mercado sobre-reacciona TRIPLEMENTE: (1) AI disruption risk is LESS than feared for quality incumbents -- Copilot failing, ADBE AI-first ARR tripled, Doximity AI products used by 300K+ prescribers, (2) oil at $111 is CYCLICAL risk-off, not STRUCTURAL damage to software (these companies have 80-90% gross margins, zero commodity exposure), y (3) tariffs barely affect pure software (confirmed after 1 year of Liberation Day). ADBE at P/E 14.0x near 52wL, Q1 BEAT, FTC settled, CEO transition is orderly. GDDY at P/E 13.4x near 52wL. The market is pricing in macro armageddon for companies that are literally the LEAST exposed to oil/tariffs. Quality compounders in software are the best risk/reward in the market right now -- they are trading at cyclical lows for cyclical reasons while their structural tailwinds (AI spend, enterprise digitalization) are accelerating."

**Flujos de fondos (12m):** Inflows masivos en energy/commodities, outflows de software SaaS puro. Institutional underweight = structural buying opportunity when risk clears.

---

## Nuestras Posiciones Actuales

### ADBE - Adobe Inc. (Creative Software) -- POSITION 8.1%

- **Holding:** 3.83 shares, ~EUR 1,014 invested
- **Quality Score:** 80 Tool / 77 adj (Tier A)
- **Current Price:** $240.14 (T1 Apr 7). Near 52wL $233.16. P/E 14.0x.
- **Thesis:** Lider indiscutible en software creativo profesional. 89% gross margin, 53% ROIC, $9.8B FCF. Integrando AI via Firefly exitosamente (AI-first ARR tripled YoY, Firefly subs +75% QoQ, video gen +8x YoY). Mercado sobrereacciono por miedo a AI disruption + war risk-off + CEO transition.
- **Q1 FY2026 RESULTS (Mar 12):** BEAT. Revenue $6.4B (+11% YoY, beat $6.28B est). EPS $6.06 (beat $5.87). MAU >850M (+17% YoY). Creative freemium MAU 80M (+50% YoY). Firefly Enterprise new customer acquisition +50% YoY. Cash from operations $2.96B (Q1 record).
- **FY2026 GUIDANCE UPPED (Mar 12):** Revenue $25.9-26.1B. Non-GAAP EPS $23.30-23.50. Q2 guide: revenue $6.43-6.48B, EPS $5.80-5.85. This is ABOVE prior consensus.
- **CEO TRANSITION (Mar 12):** Narayen stepping down after 18 years. Stays as Board Chair. Search committee led by Frank Calderoni. Internal and external candidates. Consensus analyst PT ~$344-366 (44-52% upside from current). Shares still depressed post-announcement.
- **FTC:** SETTLED ($150M total, trial CANCELLED). KC#6 RESOLVED.
- **AI RECKONING ASSESSMENT (Apr 7):** ADBE remains PARTIALLY INSULATED. Creative tools are outcome-based vs seat-based. Firefly's training on licensed content is defensible moat. AI-native ARR growing 3x. MCP/agentic AI acceleration is POSITIVE for creative tool usage (more agent-created content needs professional finishing). No CMA UK investigation found in searches.
- **Por que nos gusta:**
  - Quality Compounder a precio de deep value stock (P/E 14.0x, -43% de maximos)
  - E[CAGR] ~24%+ at current price >> 12% threshold
  - AI es OPORTUNIDAD: Firefly AI-first ARR tripled, video gen 8x, 850M MAU
  - FCF margin 41% (excepcional). Q1 record FCF $2.96B.
  - Guidance UPPED post-Q1 -- operational execution strong despite CEO transition
  - Copilot's failure to gain traction REDUCES bundle-threat to Adobe's point solutions
  - FTC RESOLVED -- #1 contrathesis risk eliminated
- **Riesgos:**
  - CEO transition uncertainty -- successor quality is binary. KC#9: if successor fails + rev growth <8% 2Q
  - AI Reckoning / seat compression -- partially insulated but structural shift bears monitoring
  - Oil $111+ creating extended risk-off selling, rate cuts pushed to Q1 2027
  - Canva/Figma in prosumer segment
- **Status:** HOLD. Avg cost $261.57, FV $385 (S284: CMA UK reopens multi-jurisdiction risk), MoS ~38% at current price.
- **Thesis:** `/home/angel/value_invest2/thesis/active/ADBE/thesis.md`

### FTNT - Fortinet, Inc. (Cybersecurity) -- SOLD Mar 27

- **SOLD:** 10.57 shares at $81.03 on 2026-03-27. P&L -3.5%.
- **Reason:** EXIT APPROVED S152. E[CAGR] worst in portfolio. Proceeds rotated.
- **Lesson:** CEO transition risk + compressed E[CAGR] = rotate to better candidates.

---

## Dependencias Activas

**Si este sector view cambia MATERIALMENTE, re-evaluar todas las thesis listadas.**

| Tipo | Ticker | Thesis Path | Ultima Eval | Status |
|------|--------|-------------|-------------|--------|
| Portfolio | ADBE | thesis/active/ADBE/thesis.md | 2026-04-07 | HOLD. $240.14 P/E 14.0x. CEO search ongoing. Q1 BEAT. Guidance upped. FV $385 (S284 revision). MoS ~38%. AI Reckoning: partially insulated. |
| Portfolio | GDDY | thesis/active/GDDY/thesis.md | 2026-03-27 | HOLD. $83.28. FV $130. QS 75 Tier A. A&C growth. Securities investigations (ambulance-chaser, no SEC action). Q1 earnings Apr 30. |
| Portfolio | ALFA.L | thesis/active/ALFA.L/thesis.md | 2026-03-27 | HOLD. 147.8p near 52wL. FV 215p. QS 83 Tier A. FY2025 strong. Ex-div Apr 30. New CFO Jul 6. |
| Sold | FTNT | thesis/archive/FTNT/ | 2026-03-27 | SOLD at $81.03 (-3.5%). Rotated. |
| Portfolio | GDDY | thesis/active/GDDY/thesis.md | 2026-04-07 | HOLD. $83.28 P/E 13.4x near 52wL. Earnings Apr 30. FY2026 ~6% rev growth. EBITDA >33%. Non-seat model insulates from AI Reckoning. |
| Portfolio | ALFA.L | thesis/active/ALFA.L/thesis.md | 2026-04-07 | HOLD. 147.80p P/E 14.8x near 52wL. FY2025 strong: rev GBP 126.7M (+17%), 32% op margin, ARR GBP 43.9M (+15%), NRR 109%. New CFO Andrew Dickson joining Jul 6. Ex-div Apr 30. |
| Watchlist | TYL | thesis/research/TYL/thesis.md | 2026-02-04 | Entry $250. Currently ~$374 -- FAR from entry. |
| Watchlist | DSY.PA | thesis/research/DSY.PA/thesis.md | 2026-02-11 | Entry EUR 15.50. Q1 Apr 15. |
| Watchlist | INTU | thesis/research/INTU/thesis.md | 2026-02-13 | Entry $285-300 (R3: FV $395, QS 72 Tier B). Currently ~$481 -- FAR. |

### Protocolo de Cambio Material

Cuando actualices este sector view con cambio MATERIAL:
1. Cambiar status a "NEEDS_REVIEW" en todas las dependencias
2. Anadir al calendario: "RE-EVAL [tickers] por cambio en [sector]"
3. Documentar el cambio en "Historial de Cambios"

**ASSESSMENT (Apr 7): This update is NOT material for sector status or position theses. Oil surge to $111 is CYCLICAL risk-off (already captured in existing thesis risk sections), not a structural change for software. AI acceleration (MCP 97M, GPT-6/Claude 5 coming) CONFIRMS existing thesis (seat compression real but our positions partially insulated). ALFA.L FY2025 results are POSITIVE (revenue +17%, margins strong) -- reinforces thesis. No NEEDS_REVIEW triggered. Sector status remains NEUTRAL.**

---

## Empresas Objetivo

### Para analisis profundo (Tier A/B potential)

| Ticker | Empresa | Razon | P/E | Priority | Notas |
|--------|---------|-------|-----|----------|-------|
| DOX | Amdocs Limited | IT services, 4 analysts only, 31% upside to targets | 14.7x | Alta | Low coverage = inefficiency |
| BR | Broadridge Financial | 32% undervalued vs FV $290, fintech B2B | ~18x | Media | Investor communications |
| G | Genpact | IT services/BPO, P/E 12.6x, low coverage (11 analysts) | 12.6x | Media | Business services overlap |
| SAIC | SAIC | Gov IT contractor, P/E 12.4x, 9 analysts | 12.4x | Media | Defense/gov tech -- Iran war TAILWIND |

### Evitar (overvalued, no moat, o value traps)

| Ticker | Empresa | Razon |
|--------|---------|-------|
| SNOW | Snowflake | P/S >15x, no profitability, hype valuation |
| PLTR | Palantir | P/E >100x, gov dependency, volatile. #2 holding in XLK at 2.8% |
| CRM | Salesforce | Good company pero P/E 25x+ no es value |
| NOW | ServiceNow | P/E >50x, priced for perfection |
| MSFT | Microsoft | Excelente empresa pero P/E 35x, no MoS. Copilot underperforming |
| DBX | Dropbox | VALUE TRAP. Structural decline, AI-commoditized FSS |

---

## Analizadas - En Watchlist

| Ticker | Empresa | FV | Entry | MoS Req | Reason | Fecha Analisis |
|--------|---------|-----|-------|---------|--------|----------------|
| TYL | Tyler Technologies | $280 | $250-260 | 20% (Tier B) | Wide moat gov software, QS 72. Currently ~$374 -- FAR from entry | 2026-02-04 |
| **DSY.PA** | **Dassault Systemes** | **EUR 21.00** | **EUR 15.50-17.00** | **20-25%** | **#1 PLM globally, QS 78 (Tier A borderline). -54% from high. Cloud transition pain. Q1 Apr 15.** | **2026-02-11** |
| **INTU** | **Intuit Inc.** | **$395 (R3)** | **$285-300** | **24-28%** | **R3 COMPLETE. QS 72 Tier B. Currently ~$481 -- FAR from entry.** | **2026-02-13** |
| **PAYC** | **Paycom Software** | **$115 (R4)** | **$88** | **23.5%** | **R4 WATCHLIST. QS 75 adj (Tier A borderline). Per-seat pricing KC#8.** | **2026-02-14** |

---

## Catalizadores Proximos

| Fecha | Evento | Impacto esperado |
|-------|--------|------------------|
| Apr 15, 2026 | DSY.PA Q1 2026 earnings | First datapoint on FY2026 3-5% guide |
| Late Apr 2026 | FTNT EXIT execution | Sell ~$83-88 range. Rotate proceeds. |
| Apr 30, 2026 | GDDY Q1 2026 earnings | Key: revenue growth trajectory, EBITDA margin, SMB health in oil/tariff environment |
| Apr 30, 2026 | ALFA.L ex-dividend date | 1.0% yield. Dividend pay date May 29. |
| May 2026 | ADBE successor announcement? | Narayen said "a few months" (Mar 12). Strong internal candidate = bullish. |
| May 19, 2026 | DOCS Q4 FY2026 earnings | Key: NRR trend (112% in Q3), AI product adoption (300K+ prescribers), FY2027 guide |
| May-Jul 2026 | GPT-6 release expected | Major AI model upgrade. Watch for enterprise pricing changes, agentic capability jumps |
| May-Sep 2026 | Claude 5 "Fennec" release expected | Agentic workflow improvements. Potential accelerator for seat compression |
| Jul 6, 2026 | ALFA.L new CFO Andrew Dickson joins | CFO designate start. Exec director from Sep 16. Transition from Duncan Magrath |
| Q1 2026 earnings season (ongoing) | Tech sector EPS +45.1% YoY expected | Strongest sector. Validates IT spend $6.15T Gartner thesis. Watch for AI Reckoning commentary. |
| H1-H2 2027? | Fed rate cuts? | If oil stays >$100, rate cuts pushed to Q1 2027 at earliest. Extended multiple compression for growth. |
| Ongoing | Agentic AI seat compression | ACCELERATING. MCP 97M installs. Agentic AI Foundation formed. Monitor "seat compression" language in all earnings calls. |
| Ongoing | Enterprise AI adoption | AI pricing power for incumbents (+15-25% ARPU uplift). $2.52T total AI spending. ADBE well-positioned. |
| Ongoing | Section 301 tariff outcomes | Probes into 16+ economies. Software mostly immune but customer uncertainty lingers. 1 year post-Liberation Day. |

---

## Historial de Cambios

| Fecha | Cambio | Razon |
|-------|--------|-------|
| 2026-02-04 | Status inicial: NEUTRAL | Sector creado. Valuaciones elevadas excepto bolsas de valor |
| 2026-02-04 | ADBE anadido a posiciones | Compra aprobada, primer Quality Compounder tech |
| 2026-02-04 | TYL analizado -> Watchlist | QS 72 (Tier B, no A). Wide moat pero overvalued. Entry $250-260 |
| 2026-02-05 | DBX analizado -> Watchlist | QS 73 pero 5/10 value trap factors. Entry agresivo $18-20 requerido |
| 2026-02-06 | BYIT.L comprado - Sixth Tier A | UK IT services VAR, QS 81, MoS 35%. Fallen angel -47% |
| 2026-02-11 | BYIT.L updated to HOLD ON PROBATION | Adversarial review: QS 68-72 Tier B, FV 345p (from 455p) |
| 2026-02-11 | DSY.PA analizado -> Watchlist | Full fundamental analysis post -21% crash. QS 78, FV EUR 21.00 |
| 2026-02-12 | INTU analizado -> Watchlist | QS 75 -> R3 QS 72 Tier B. FV $465 -> R3 $395. Insider selling concern |
| 2026-02-14 | PAYC R4 WATCHLIST. Standing order $88 | QS 75 adj. Per-seat pricing risk KC#8 |
| 2026-03-07 | BYIT.L SOLD (S143 restructuring) | QS 68 Tier B ON PROBATION. Orphan. Capital rotated to quality ADDs. P&L +5.7%. |
| 2026-03-07 | Update: Iran crisis + AI adoption data + earnings preview | Non-material. Copilot disappointing, enterprise software +15%, war risk-off temporary |
| 2026-03-15 | FOMC prep refresh | Oil $99, FOMC context, ADBE HOLD through FOMC |
| 2026-03-18 | MATERIAL UPDATE: ADBE CEO transition + Q1 BEAT + FOMC day + tariffs | Narayen stepping down. Q1 BEAT. FTC SETTLED. FTNT EXIT approved. Fed holds. Section 301 probes. |
| 2026-03-30 | AI Reckoning structural update + IEEPA ruling + ALFA.L CFO | MSCI Software -21% YTD. Seat compression confirmed. IEEPA tariffs ruled unlawful. |
| **2026-04-07** | **Incremental refresh: oil $111 WTI, ALFA.L FY2025 results, AI acceleration, Liberation Day 1yr** | **Oil surged from $94 to $111.50 -- CYCLICAL risk-off intensified but NO structural impact on software. ALFA.L FY2025 strong (rev +17%, 32% margin, NRR 109%). New CFO Dickson joining Jul 6. MCP 97M installs, agentic AI entering production. Liberation Day 1yr: software confirmed immune. ADBE $240 P/E 14x, GDDY $83 P/E 13.4x -- both near 52wL. Sector NEUTRAL unchanged. Non-material update -- existing KCs and risk assessments already capture all themes.** |

---

## Historial de Analisis (Archivadas)

| Ticker | Razon Archivo | Fecha | Leccion |
|--------|---------------|-------|---------|
| BYIT.L | SOLD S143. QS 68 Tier B ON PROBATION. Orphan, no platform. Capital rotated. P&L +5.7%. | 2026-03-07 | Insider ownership matters (claimed 9.6%, actual 0.4%). Microsoft dependency = structural risk. |
| DBX | Removed from watchlist. VALUE TRAP confirmed. AI-commoditized FSS. | 2026-03-18 | FSS without moat = AI-disrupted first. Users declining -1.5%/yr. |

---

## Reglas de Ciclo de Vida

### Cuando una empresa se COMPRA:
- Mover de "Empresas Objetivo" o "Watchlist" a "Nuestras Posiciones Actuales"
- Anadir a "Dependencias Activas" como Portfolio

### Cuando una thesis se ARCHIVA (vendida o descartada):
- Mover de "Dependencias Activas" a "Historial de Analisis"
- Registrar razon y fecha de archivo
- Mantener maximo 10 entradas en historial, luego purgar

### Cuando sector view crece demasiado (>300 lineas):
- Mover "Historial de Analisis" a archivo separado: world/sectors/archive/technology-history.md
- Mantener solo ultimos 6 meses en "Analizadas - Watchlist"

---

## Fuentes Principales

### Sector-Level (Mar-Apr 2026)
- [Gartner: IT Spending $6.15T in 2026 (+10.8%)](https://www.gartner.com/en/newsroom/press-releases/2026-02-03-gartner-forecasts-worldwide-it-spending-to-grow-10-point-8-percent-in-2026-totaling-6-point-15-trillion-dollars)
- [Gartner: AI Spending $2.52T in 2026 (+44%)](https://www.gartner.com/en/newsroom/press-releases/2026-1-15-gartner-says-worldwide-ai-spending-will-total-2-point-5-trillion-dollars-in-2026)
- [SaaStr: Enterprise Software Spend +15.2%](https://www.saastr.com/gartner-enterprise-software-spend-will-grow-a-stunning-15-2-next-year-but-most-of-that-will-go-to-price-increases-and-ai-apps/)
- [Deloitte: 2026 Software Industry Outlook](https://www.deloitte.com/us/en/insights/industry/technology/technology-media-telecom-outlooks/software-industry-outlook.html)
- [Motley Fool: Enterprises Spending 15% More on Software Thanks to AI](https://www.fool.com/investing/2026/03/01/spending-more-ai-software-how-to-profit-msft/)

### SaaSpocalypse / Software Selloff
- [CNBC: AI Fears Pummel Software Stocks](https://www.cnbc.com/2026/02/06/ai-anthropic-tools-saas-software-stocks-selloff.html)
- [Fortune: BofA Says Tech Selloff "Doesn't Make Sense"](https://fortune.com/2026/02/04/why-saas-stocks-tech-selloff-freefall-like-deepseek-2025-overblown-paradox-irrational/)
- [SaaStr: The 2026 SaaS Crash](https://www.saastr.com/the-2026-saas-crash-its-not-what-you-think/)
- [Benzinga: 2026 Market Rotation -- Long Energy, Short Software](https://www.benzinga.com/markets/economic-data/26/02/50354156/2026-market-reversal-long-energy-oil-services-short-software-ai-rewrites-regimes)

### AI / Copilot Adoption / Agentic AI
- [Recon Analytics: Why Licenses Don't Equal Adoption](https://www.reconanalytics.com/ai-choice-2026-why-licenses-dont-equal-adoption/)
- [Gartner: Strategic Predictions for 2026 (Trough of Disillusionment)](https://www.gartner.com/en/articles/strategic-predictions-for-2026)
- [SaaS Statistics 2026: AI-native growing 2x](https://www.hostinger.com/tutorials/saas-statistics)
- [Enterprise Agentic AI Landscape 2026](https://www.kai-waehner.de/blog/2026/04/06/enterprise-agentic-ai-landscape-2026-trust-flexibility-and-vendor-lock-in/)
- [AI Model Releases April 2026](https://blog.mean.ceo/new-ai-model-releases-news-april-2026/)
- [Anthropic Claude News April 2026](https://blog.mean.ceo/anthropic-claude-news-april-2026/)

### Adobe Q1 FY2026 + CEO Transition
- [CNBC: Adobe CEO Narayen to Step Down](https://www.cnbc.com/2026/03/12/adobe-ceo-shantanu-narayen-step-down.html)
- [Adobe Press Release: Leadership Update](https://news.adobe.com/news/2026/03/leadership-update)
- [Futurum Group: Adobe Q1 FY2026 AI Monetization Progress](https://futurumgroup.com/insights/adobe-q1-fy-2026-earnings-show-ai-monetization-progress-amid-ceo-transition/)
- [Adobe ADBE Analyst Forecast](https://stockanalysis.com/stocks/adbe/forecast/)

### ALFA.L FY2025 Results
- [Finance Connect: Alfa reports GBP 126.7M revenue as SaaS growth accelerates](https://finance-connect.com/alfa-reports-126-7m-revenue-as-saas-growth-accelerate/)
- [Sharecast: Alfa Financial Software reports full-year growth](https://www.sharecast.com/news/news-and-announcements/alfa-financial-software-reports-full-year-growth-shares-slide--22021040.html)
- [Investegate: Appointment of CFO Andrew Dickson](https://www.investegate.co.uk/announcement/rns/alfa-financial-software-holdings--alfa/appointment-of-chief-financial-officer/9426852)

### Oil / Macro
- [EIA Short-Term Energy Outlook](https://www.eia.gov/outlooks/steo/)
- [WTI Crude Oil Price Analysis April 7 2026](https://fxdailyreport.com/wti-crude-oil-price-analysis-for-april-7-2026/)
- [Fortune: Current Price of Oil April 3 2026](https://fortune.com/article/price-of-oil-04-03-2026/)

### Tariffs / Section 301
- [CSIS: Liberation Day Tariffs Explained](https://www.csis.org/analysis/liberation-day-tariffs-explained)
- [Marketplace: How Liberation Day Tariffs Affected the Economy 1 Year Later](https://www.marketplace.org/story/2026/04/02/how-liberation-day-tariffs-have-affected-the-economy-in-a-year/)
- [TechTarget: How US Tariffs Reshaping Tech Landscape](https://www.techtarget.com/whatis/feature/How-US-tariffs-are-reshaping-the-tech-landscape)

### FOMC March 2026
- [CNBC: Fed Interest Rate Decision March 2026](https://www.cnbc.com/2026/03/17/the-fed-issues-its-latest-interest-rate-decision-wednesday-heres-what-to-expect.html)

### Cloud Infrastructure
- [Synergy Research: Cloud Market Share](https://www.srgresearch.com/articles/cloud-market-share-trends-big-three-together-hold-63-while-oracle-and-the-neoclouds-inch-higher)

---

*Proxima revision: 2026-04-15 (DSY.PA Q1 earnings + FTNT exit window + GDDY/ALFA.L pre-earnings) o ante cambio material*
