# Sector: Animal Health

> Ultima actualizacion: 2026-03-21
> Status: SOBREPONDERAR (selectivo — calidad en drawdown por problema idiosincratico)

## Resumen Ejecutivo

Animal health es un oligopolio de alta calidad con barreras regulatorias masivas, baja sensibilidad a recesion (la gente NO deja de tratar a sus mascotas), y crecimiento secular impulsado por pet humanization. El TAM global ~$67-78B crece al 7-11% CAGR, con companion animal (mascotas) representando ~65% del mercado y creciendo mas rapido que livestock.

El sector esta en un momento inusual: la empresa dominante (Zoetis, ~23% market share global) ha caido -32% desde maximos por un problema idiosincratico — preocupaciones de seguridad con Librela (su farmaco de dolor articular en perros). Esto ha arrastrado el P/E de ~35x historico a 19x. El RESTO del negocio (parasiticides, dermatologia, diagnostics) sigue creciendo a 5-8% organico.

Mi tesis: el mercado esta sobrepenalizando ZTS por Librela. Los margenes siguen expandiendose (OP 36→38%), el FCF es record ($2.3B), y el pipeline de companion animal es robusto. Si Librela se estabiliza (incluso a niveles reducidos), el negocio core justifica un P/E de 25-30x. A 19x, el mercado esta pricing-in un deterioro permanente que es improbable dado el moat regulatorio y la diversificacion del portfolio.

---

## Metricas Clave

| Metrica | Valor | vs Historia | Tendencia |
|---------|-------|-------------|-----------|
| TAM Global | $67-78B (2026) | | Creciendo |
| CAGR (5y proyectado) | 7-11% | Acelerando vs 5% historico | ↑ |
| P/E sector (ZTS proxy) | 19x | vs 35x (5y avg) | Deprimido |
| Companion animal mix | ~65% revenue | Era ~50% en 2015 | ↑ Secular |
| Pet humanization spend/pet | Creciendo 3-5%/yr | | ↑ |

---

## Estructura Competitiva

**Concentracion:** Oligopolio estable (top 5 = ~60% market share)
**Top 5 market share:** ~60%

| Empresa | Market Share | Fortaleza |
|---------|--------------|-----------|
| Zoetis (ZTS) | ~23% | Companion animal + livestock. WIDE moat. ROIC 26%. |
| Merck Animal Health | ~14% | Companion (Bravecto) + livestock. Subsidiary of MRK. |
| Boehringer Ingelheim | ~12% | NexGard franchise. Private — not investable directly. |
| Elanco (ELAN) | ~8% | Spun from Lilly 2020. Lower quality. Debt-heavy. |
| IDEXX (IDXX) | ~6% | Diagnostics dominant. Not pharma. QS 83. Separate thesis. |

**Barreras de entrada:** MUY ALTAS
- Regulatory approval takes 5-7 years per product (FDA Center for Veterinary Medicine)
- Manufacturing scale (biologics especially) = billions in capex
- Veterinarian relationships = decades of trust
- Patent cliffs are SLOW in animal health (20-40% erosion over 5 years, not human pharma's patent cliff)
- Data/clinical evidence moat deepens over time

---

## Ciclo y Sensibilidad

| Factor | Sensibilidad | Notas |
|--------|--------------|-------|
| Tipos de interes | Baja | Pet spend is non-discretionary. Livestock = food chain. |
| Recesion | Baja-Media | Companion: very resilient (pet humanization). Livestock: mild cyclicality. |
| Inflacion | Baja | Pricing power historically 3-5%/yr. Vets pass through. |
| Oil/energy | Muy Baja | Not input-dependent. |

**Mejor fase del ciclo:** All-weather (companion animal), Late-cycle sensitive (livestock slightly)
**Beta tipico:** 0.8-1.0

---

## Disrupcion y Riesgos

### Tecnologicos
| Amenaza | Probabilidad | Impacto | Timeline |
|---------|--------------|---------|----------|
| GLP-1 for pets (Okava OKV-119) | Baja-Media | Bajo (complementary, not substitutive) | MEOW-1 cat trial dosing underway. Results expected summer 2026. Dog trial to follow. FDA approval ~2028. 6-month implant, ~$100/mo. Does NOT compete with ZTS pain/parasiticide portfolio — different indication (obesity) |
| AI diagnostics (IDEXX) | Media | Bajo para pharma (helps diagnostics) | En progreso |
| Telehealth vet | Baja | Bajo (prescriptions still needed) | 2025+ |

### Regulatorios
| Regulacion | Probabilidad | Impacto |
|------------|--------------|---------|
| FDA adverse event scrutiny (Librela) | ACTIVO | ALTO — driving ZTS selloff. Label updated Feb 2025 with post-approval experience section. 25M+ doses globally, AE rate <10/10,000 (rare). |
| **Apoquel US patent expiry Nov 2026** | **CERTAIN** | **MEDIO-ALTO — $1.0B+ revenue at risk. Generic oclacitinib expected post-expiry. EU patent already expired Sep 2024. Key monitoring point for ZTS thesis.** |
| EU pharmacovigilance changes | Baja | Medio |
| Antibiotic resistance regulations | Media | Medio — livestock impact |

### Competitivos
- **Nuevos entrantes:** Almost impossible in branded pharma. Biosimilars 5+ years away for key products.
- **Sustitutos:** Generic erosion SLOW (20-40% over 5 years). Pet insurance growth = more treatments, not fewer.

---

## Sentimiento de Mercado

**Sentimiento actual:** ODIADO (especificamente ZTS por Librela)

**Narrativa dominante:**
"Librela tiene problemas de seguridad graves (musculoskeletal adverse events 9x vs comparadores). ZTS sobre-dependia de este blockbuster. Guidance 3-5% para 2026 es mediocre."

**Mi contra-tesis:**
"Librela es un producto de ~$300M en un negocio de $9.5B. Incluso si Librela goes to ZERO, el negocio core crece 5-6% organico con margenes expandiendose. A P/E 19x vs 35x historico, el mercado esta pricing-in un deterioro permanente del negocio core que la evidencia NO soporta. Simparica ($1.5B, +12%), Apoquel/Cytopoint ($1.7B, +6%), y diagnostics siguen siendo best-in-class."

**Flujos de fondos (12m):** Outflows significativos. Institutional selling en Q3-Q4 2025 post-Librela report.

---

## Nuestras Posiciones Actuales

*Ninguna actualmente. ZTS en pipeline como R1 PRIORITY.*

---

## Dependencias Activas

**Si este sector view cambia MATERIALMENTE, re-evaluar todas las thesis listadas.**

| Tipo | Ticker | Thesis Path | Ultima Eval | Status |
|------|--------|-------------|-------------|--------|
| Pipeline | ZTS | (pending R1) | 2026-03-13 | R1 PRIORITY |

### Protocolo de Cambio Material
Cuando actualices este sector view con cambio MATERIAL:
1. Cambiar status a "NEEDS_REVIEW" en todas las dependencias
2. Anadir al calendario: "RE-EVAL [tickers] por cambio en [sector]"
3. Documentar el cambio en "Historial de Cambios"

---

## Empresas Objetivo

### Para analisis profundo
| Ticker | Razon | Priority |
|--------|-------|----------|
| ZTS | R1 COMPLETE. QS 78 Tier A, FV $130, entry $100-110. WATCHLIST. Double franchise headwind (Librela + Apoquel Nov 2026 patent). Needs Q1 May. | DONE — WATCHLIST |
| IDXX | Diagnostics monopoly, QS 83, but P/E ~45x still | Media |
| ELAN | Low quality (debt-heavy), but deep value play | Baja |

### Evitar
| Ticker | Razon |
|--------|-------|
| Boehringer Ingelheim | Private — not investable |
| Merck Animal Health | Part of MRK — can't isolate animal health exposure |

---

## Analizadas - En Watchlist

| Ticker | Empresa | FV | Entry | MoS Req | Thesis | Fecha Analisis |
|--------|---------|----|----- -|---------|--------|----------------|
| ZTS | Zoetis | $130 | $95-100 | ~25% | R1 Complete, SO $95 GATED on Q1 earnings May 2026 | 2026-03-13 |

---

## Catalizadores Proximos

| Fecha | Evento | Impacto esperado |
|-------|--------|------------------|
| 2026-03-21 | ZTS ~$116 near 52wL ($114.47) | POSITIVE — entry opportunity if Q1 clears Librela fears |
| **2026-11 (est)** | **Apoquel US patent expiry** | **KEY — generic competition begins. Monitor erosion pace vs ZTS guidance** |
| Q2 2026 | ZTS Q1 2026 earnings (~May) | KEY — Librela trajectory, guide update |
| 2026 summer | Okava MEOW-1 results (cat GLP-1 trial) | MONITOR — if positive, dog trial next. FDA approval ~2028. Complementary to ZTS, not substitutive |
| 2027-28 | Okava OKV-119 FDA filing | MONITOR — pet GLP-1 |
| 2026 ongoing | FDA Librela pharmacovigilance | RISK — could worsen or stabilize |

---

## Historial de Cambios

| Fecha | Cambio | Razon |
|-------|--------|-------|
| 2026-03-21 | Mini-update: Apoquel patent Nov 2026, Librela label updated, MEOW-1 dosing, ZTS watchlist | Apoquel US patent expiry confirmed Nov 2026 (generic risk). Librela FDA label updated Feb 2025 (AE rare). Okava MEOW-1 cat trial underway. ZTS added to Analizadas |
| 2026-03-13 | Status inicial: SOBREPONDERAR (selectivo) | ZTS fallen angel at 52wL, sector quality + idiosyncratic opportunity |

---

## Fuentes

- [Zoetis Q4 2025 Full Year Results](https://www.businesswire.com/news/home/20260211839882/en/Zoetis-Reports-Fourth-Quarter-and-Full-Year-2025-Results)
- [Zoetis Q4 2025 Slides: 6% Growth Achieved, 3-5% Projected 2026](https://www.investing.com/news/company-news/zoetis-q4-2025-slides-6-annual-growth-achieved-35-projected-for-2026-93CH-4503431)
- [ZTS Falls on Q3 Librela Weakness](https://www.insidermonkey.com/blog/zoetis-zts-falls-following-q3-disappointment-and-librela-demand-weakness-1697689/)
- [ZTS Stock -24.8% YTD Analysis](https://finance.yahoo.com/news/zoetis-stock-plummets-24-8-144700688.html)
- [Zoetis Building a Moat in $112B Market](https://www.ainvest.com/news/zoetis-building-moat-112-billion-market-ensure-minimum-rivalry-2601/)
- [GLP-1 for Cats: Clinical Trial](https://abcnews.go.com/GMA/Wellness/weight-loss-drugs-cats-company-launches-clinical-trial/story?id=128054236)
- [Okava MEOW-1 First Cat Dosed](https://www.cbs42.com/business/press-releases/ein-presswire/871966123/okava-announces-first-cat-dosed-in-meow-1-study-of-okv-119)
- [Apoquel Patent Expiration Analysis](https://pharsight.greyb.com/drug/apoquel-patent-expiration)
- [Librela Label Update Feb 2025](https://news.zoetis.com/press-releases/press-release-details/2025/Zoetis-Announces-U.S.-Label-Update-for-Librela)
- [FDA Dear Veterinarian Letter - Librela](https://www.fda.gov/animal-veterinary/product-safety-information/dear-veterinarian-letter-notifying-veterinarians-about-adverse-events-reported-dogs-treated-librela)
- [Animal Health Market Size 2026-2035](https://www.researchnester.com/reports/animal-health-market/1225)
