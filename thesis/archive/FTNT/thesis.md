# FTNT - Fortinet, Inc.

> **Fair Value:** $88 (R3 post-DA resolution)
> **Expected Growth:** 8% (R3 post-DA. Billings +18%, SASE +40%, rev guide +12%. Buyback ~2%.)
> Pipeline Stage: EXIT APPROVED S152. Committee APPROVE. Late April sell. SPGI rotation target.
> R1 Fundamental Analysis | Date: 2026-02-26 (header refreshed 2026-03-21)
> Analyst: fundamental-analyst (opus)
> Sector View: world/sectors/cybersecurity.md (FRESH, same day)
> Stage: R4 APPROVED — POSITION OPENED 2026-02-26 (5.98 shares @ $79.12, EUR 400)
> EXIT: APPROVED S152 (2026-03-13). E[CAGR] ~9% at $81.40, 3 KCs MONITORING, ~7.5% MoS. Rotate to SPGI EUR 400 + best available EUR 375. Late April timing (Q1 earnings Apr 30).
> **Current Price (2026-03-21):** $81.40. Down from $83.67 at last update. MoS vs FV $88 = 7.5%. Exit plan CONFIRMED: sell late April pre-Q1 or post-Q1 depending on price action. Price hikes (5-20%) effective Mar 2 = modest positive.
> **Bear Case:** CVE vulnerability pattern escalating (11 new vulns Mar 10, CISA advisory). Deferred revenue growth decelerating. E[CAGR] 9.8% worst in portfolio. EXITING April — contrathesis supports exit decision.

---

## TL;DR

Fortinet is the #1 firewall vendor globally (55% unit share) with a proprietary ASIC hardware moat, 83% ROIC, ~32% FCF margins, and net cash. Trading near 52-week low ($77.35 vs $70.12 low) due to firewall refresh cycle deceleration fears and SaaSpocalypse contagion. FY2025 results were strong: revenue +14%, billings +18%, SASE billings +40%. FY2026 guidance of $7.5-7.7B revenue (+12%) and $2.94-3.00 non-GAAP EPS. The ASIC cost advantage, platformization tailwind, and secular cybersecurity spending growth create a durable compounder opportunity at a reasonable multiple for the sector. At $77.35, the stock offers ~19% MoS to my Base FV of $93-96, with E[CAGR] of ~16%.

**POST-DA (R3 Resolution):** FV revised $96 to $88 (-8.3%). QS 88 to 85adj. STRONG COUNTER from DA with 3 CRITICAL findings: (1) refresh cycle FADING not catalyst, (2) product security crisis omitted (CVE-2025/2026), (3) thin edge vs consensus. All resolved in R3. WATCHLIST with SO $73 approved R4. SO raised to $78 in S126. Bought at $79.12 (essentially at market) as 5th E[CAGR]-framework buy.

---

## Quality Score

```
QS Tool: 87/100 (Tier A) — updated Mar 7, was 70 when ROIC data gap existed
QS Adjusted: 87/100 (Tier A) — no manual adjustment needed

Previous adjustment of +15 is NO LONGER NEEDED because:
  - FY2025 ROIC data (65.0%) now available in yfinance → tool scores ROIC natively
  - ROIC-WACC spread +54.9pp correctly captured by tool (was NaN)
  - Market Position still requires manual input but rounding makes adjustment immaterial

Tool correctly captures:
  - ROIC Spread: 15/15 (ROIC 65%, spread +54.9pp vs WACC 10.1%)
  - FCF Margin 10/10 (~32.7% margin)
  - Leverage 10/10 (net cash $2.5B)
  - FCF Consistency 5/5 (4/4 years positive)
  - Revenue CAGR 10/10 (+15.5%)
  - EPS CAGR 10/10 (+31.4%)
  - GM Trend 5/5 (expanding: 75.4% -> 76.7% -> 80.6% -> 80.5%)
  - GM Premium 10/10 (+25.5pp vs sector median)
  - Insider Ownership 5/5 (17.4% — CEO Ken Xie 8.3%, CTO Michael Xie 7.4%)
```

**QS Tool: 87/100 | QS Adjusted: 87/100 -- Tier A (Quality Compounder)**

---

## Business Understanding

### What Fortinet Does

Fortinet is a cybersecurity platform company founded in 2000 by brothers Ken and Michael Xie. The company designs, develops, and sells cybersecurity solutions centered around the FortiGate next-generation firewall (NGFW), powered by proprietary purpose-built ASICs (Application-Specific Integrated Circuits). This hardware advantage delivers 5-10x better price/performance than competitors using general-purpose CPUs, creating a structural cost moat that is effectively impossible to replicate without billions in R&D investment and 20+ years of iteration.

The business model has evolved from a pure firewall appliance vendor into a converged networking + security platform. FortiOS, a single operating system, unifies firewall, SD-WAN, SASE, ZTNA, EDR, SIEM, OT security, and more into one platform. This is critical because the cybersecurity industry is undergoing platformization: enterprises average 83 security tools from 29 vendors and are consolidating toward 2-3 platform vendors. Fortinet's single-OS architecture makes it the most natural consolidation target, particularly for organizations that want both networking and security from one vendor.

### Revenue Model

**FY2025 Revenue: $6.8B (+14% YoY)**

| Segment | FY2025 Revenue | % of Total | Growth |
|---------|---------------|------------|--------|
| Product (FortiGate + appliances) | $2,218M | 33% | +16% |
| Services (FortiGuard + FortiCare) | $4,581M | 67% | +13% |
| **Total** | **$6,800M** | **100%** | **+14%** |

The services revenue is highly recurring, composed of:
- **FortiGuard:** Subscription security services (threat intelligence, IPS, sandboxing, web filtering, etc.). Attached to every FortiGate appliance. Duration: 1-5 year contracts.
- **FortiCare:** Technical support and maintenance contracts. Near-100% attach rate.
- **FortiSASE/SecOps/OT subscriptions:** Newer, faster-growing categories.

**Deferred Revenue: $7.1B** ($3.64B current + $3.48B non-current) -- this is 104% of trailing revenue, providing exceptional visibility. Deferred revenue grew 15% YoY.

**Billings: $7.55B (+16% YoY)** -- billings are the leading indicator. Q4 billings of $2.37B grew 18%, showing acceleration. Unified SASE billings grew 40% in Q4. This acceleration is a key signal.

### Unit Economics

- **Gross Margin: 81.3%** (FY2025) -- expanding from 75.4% in FY2022. The ASIC advantage means Fortinet's COGS on hardware is materially lower than competitors, and as services mix increases (higher margin), gross margin expands.
- **SBC/Revenue: 4.3%** (FY2024) -- CRITICALLY LOW for cybersecurity. Peers: CRWD ~20%, PANW ~18%, ZS ~25%+, S ~60%. This is one of the most underappreciated advantages. SBC-adjusted FCF is close to headline FCF, unlike peers where 15-25% SBC creates massive real dilution.
- **FCF Margin: 32.5%** (FY2025, $2.21B) -- consistent 30-33% range over 4 years. High quality FCF with minimal SBC distortion.
- **Operating Margin: 35.5%** non-GAAP (FY2025) -- expanding from 21.9% in FY2022. Significant operating leverage as services mix grows.
- **Share Count: 748M diluted** (Q4 2025 vs 764.6M FY2025 average) -- active buyback program. $1.38B remaining authorization through Feb 2027. Shares DECLINING despite tech sector norm of dilution.

### Capital Intensity: Asset-Light

- Capex/Revenue: ~5% (capex includes data center buildout for FortiSASE)
- Minimal working capital needs (deferred revenue > receivables)
- Capital allocation: aggressive buybacks ($10.25B cumulative authorization), no dividend, strategic R&D investment
- Net Cash: $2.6B ($2.5B cash + $1.4B investments - $1.0B debt)

---

## Why It's Cheap: Narrative vs Reality

### Market Narrative

FTNT trades at $77.35, down 31% from its 52-week high of $111.31 and within 10% of its 52-week low of $70.12. The market narrative centers on:

1. **Firewall refresh cycle deceleration**: FY2024 product revenue was weak (hardware refresh cycle matured). Market fears this weakness extends.
2. **SaaSpocalypse contagion**: The broader software selloff ($800B evaporated) dragged FTNT down. AI disruption narrative hits all software/tech.
3. **PANW platformization leadership**: Palo Alto's aggressive bundling (CyberArk acquisition, XSIAM) is perceived as winning the platform war.
4. **Multiple compression**: The rate-driven re-rating from growth multiples to value multiples.
5. **Insider selling**: Heavy 10b5-1 plan selling ($62.6M in 90 days) creates headline noise.

### R3 Post-DA Corrections

**Refresh cycle: FADING tailwind, not catalyst.** DA correctly identified that 40-50% of the refresh cycle was complete by Q2 2025. Product revenue ex-refresh was flat/negative (KeyBanc). Q4 +20% is likely peak, not beginning. Already priced (-31% from highs).

**Product security crisis (CVE-2025/2026).** 600+ FortiGate devices compromised across 55 countries. CVSS 9.1-9.8. CISA emergency guidance. FortiCloud SSO suspended Jan 26, 2026. NOT a KC trigger (industry precedent: CrowdStrike July 2024, PANW CVE-2024-3400 -- both recovered). Monitoring via KC #6.

**Edge vs consensus is thin.** FV $88 is ~4% above consensus $84-91. Specific edge: SBC quality advantage (4.3% vs 15-30%) and ASIC moat durability. Moderate, not strong.

### Value Trap Checklist

**Value Trap Score: 0/10 -- ZERO risk factors**

---

## Competitive Position and Moat

### Moat Assessment: WIDE (85% confidence)

**Primary sources:**
1. **Proprietary ASIC Technology** -- 5-10x price/performance vs x86 competitors. 25 years of cumulative iteration. No competitor attempting replication.
2. **FortiOS Platform Lock-in** -- Single OS across firewall, SASE, EDR, SIEM, OT. Deepening as customers adopt more modules.
3. **Installed Base** -- 55% firewall unit share. $7.1B deferred revenue (104% of trailing revenue).

---

## Valuation (Post-DA R3)

| Method | Fair Value | Weight |
|--------|-----------|--------|
| Owner Earnings Yield (2.5% target) | $93 (pre-adj) | 60% |
| Forward P/E (30x FY2026-27 blend) | $86 (pre-adj) | 40% |
| **Post-DA Weighted FV** | **$88** | **100%** |

DA adjustments: -$3 (refresh fading), -$2 (security crisis), -$3 (edge thinness).

### Scenarios

| Scenario | FV | Probability |
|----------|----|-------------|
| Bear | $65 | 25% |
| Base | $88 | 50% |
| Bull | $123 | 25% |

---

## Kill Conditions (9 total, DA S149 added KC#8-9)

1. Revenue growth <8% for 2 consecutive quarters
2. Unified SASE ARR growth <15%
3. FCF margin <25% sustainably
4. SEC formal investigation from class action allegations
5. Either Xie brother <5% ownership
6. Second major security incident within 6 months causing visible customer attrition
7. Product revenue decline >10% for 2Q AND SASE growth <15% simultaneously
8. Deferred revenue growth <8% for 2 consecutive quarters (leading indicator of revenue deceleration — DA S149)
9. Third CISA KEV entry within 12 months (CVE pattern escalation — DA S149)

### Kill Conditions Status (2026-03-13, post-Investor Day + CVE update)

| KC# | Condition | Status | Notes |
|-----|-----------|--------|-------|
| 1 | Revenue growth <8% 2Q | **CLEAR** | FY2025 revenue +14%. FY2026 guide +12%. No concern. |
| 2 | Unified SASE ARR <15% | **CLEAR** | Unified SASE billings +40% Q4. Well above threshold. KEY metric for Investor Day. |
| 3 | FCF margin <25% | **CLEAR** | FCF margin 32.5%, stable 4 years. No concern. |
| 4 | SEC formal investigation | **CLEAR** | Class action early stage. No SEC involvement. LOW probability. |
| 5 | Either Xie brother <5% ownership | **CLEAR** | Ken Xie 8.3%, Michael Xie 7.4%. Well above 5%. Monitor 10b5-1 selling. |
| 6 | 2nd major security incident in 6mo | **MONITORING ELEVATED** | CVE pattern escalating: 11 new vulns Mar 10 (same day as investor briefing), active exploitation confirmed (CVE-2025-59718/59719, CVE-2026-24858), MFA bypass CVE-2026-22572. Not yet customer-attrition-level but PATTERN is concerning. |
| 7 | Product rev -10% 2Q + SASE <15% | **CLEAR** | Product revenue +16% FY2025, SASE +40%. Both strong. |
| 8 | Deferred rev growth <8% 2Q | **MONITORING** | Deferred rev +11% — above 8% but decelerating (21%→15%→11%). Approaching territory. |
| 9 | 3rd CISA KEV entry in 12mo | **MONITORING ELEVATED** | CISA issued guidance on CVE-2026-24858 (Mar 10). Need precise 12mo KEV count. 24 total entries historically, 8 in 2025 alone. If 3rd 2026 KEV confirmed → TRIGGERED. |

---

## Risk Monitoring

**Securities Class Action (Oct 2025):** Robbins Geller Rudman & Dowd filed securities fraud class action alleging misleading statements about product security vulnerabilities (CVE-2025/2026 series). Status: early stage, no SEC involvement. KC#4 tracks escalation path -- triggers only if SEC opens formal investigation. Class actions in cybersecurity are common post-incident (CrowdStrike faced similar post-Jul 2024). Monitor for: SEC subpoena, discovery outcomes, settlement size. Current assessment: LOW probability of KC#4 trigger -- no SEC interest, strong company cash position ($2.6B) for defense/settlement.

---

## E[CAGR] at Purchase Price ($79.12)

```
E[CAGR_3yr] = (FV/Price)^(1/3) - 1 + Sustainable_Growth + Dividend_Yield

FV = $88 (Post-DA Base)
Price = $79.12
Dividend = 0%

Capital Appreciation: (88/79.12)^(1/3) - 1 = 3.6%
Expected Growth: 8%
Buyback yield: +2%

E[CAGR_3yr] = ~13.6%
```

## E[CAGR] at Current Price ($83.67, as of 2026-03-07)

```
FV = $88 (Post-DA Base, unchanged)
Price = $83.67

Capital Appreciation: (88/83.67)^(1/3) - 1 = 1.7%
Expected Growth: 8%
Buyback yield: +2%

E[CAGR_3yr] = ~11.7%
MoS = (88-83.67)/88 = 4.9%
```

**E[CAGR] of 9.7% (per forward_return.py) is 2nd worst in portfolio. The thin MoS is the core issue -- FV needs to increase or price needs to drop for this position to justify its capital allocation vs alternatives.**

---

---

## 9. INVESTOR DAY FRAMEWORK (Mar 10, 2026)

### Event Details

**Fortinet Accelerate 2026 Investor Briefing**
- Date: Tuesday, March 10, 2026 at 3:00 PM PT (6:00 PM ET / 11:00 PM CET)
- Location: Mandalay Bay Convention Center, Las Vegas (part of Accelerate 2026, March 9-13)
- Format: Listen-only live webcast via investor.fortinet.com
- Content: Fortinet Security Fabric, product security practices, innovation partnerships
- Customer case studies: Adobe, Lowe's, ExxonMobil sharing platform adoption stories

### Accelerate 2026 Conference Context

The broader conference (Mar 9-13) features sessions on:
- **Secure networking and unified SASE** -- direct thesis relevance
- **AI-driven OT Security** -- growth vector validation
- **FortiOS with post-quantum cryptography** -- moat durability signal
- **Cloud Security / FortiCNAPP (Lacework integration)** -- cloud competitive positioning
- **FortiSIEM/FortiSOAR automation** -- SecOps platform stickiness
- **AI/ML-based threat hunting** -- AI positioning vs CRWD/PANW

### Key Metrics to Watch

| Metric | Current (FY2025) | Bull Signal | Bear Signal | Thesis Impact |
|--------|------------------|-------------|-------------|---------------|
| **Unified SASE ARR** | +40% billings Q4 | ARR >$1B, growth >35% | Growth <25%, churn signal | KC#2 (SASE <15% = SELL) |
| **FortiOS roadmap** | FortiOS 7.x | Post-quantum crypto, AI-native features, ASIC integration | Incremental only, no differentiation | Moat durability assessment |
| **FY2027+ revenue guide** | $7.5-7.7B FY26 | 3yr CAGR >12%, $10B+ by FY28 | <10% CAGR, refresh dependency | FV sensitivity: +/-$8-12 |
| **Platformization attach rate** | ~3.5 products/customer | >4 products, >50% new deals multi-product | Flat attach, single-product dominance persists | Revenue quality, stickiness |
| **Product margin trajectory** | GM 81.3% expanding | >82% target, ASIC advantage quantified | Margin guide flat, SASE COGS rising | Bear case margin risk |
| **ASIC roadmap (SP5 successor)** | SP5 (FortiSP5, launched 2023) | SP6 details, performance benchmarks, competitive gap widened | No next-gen ASIC timeline, x86 gap narrowing | Core moat validity |
| **Security incident response** | CVE-2025/2026 crisis | Transparent post-mortem, new security ops, customer retention data | Downplay/ignore, no churn metrics | KC#6 monitoring |
| **Capital allocation** | $1.38B buyback auth | New auth, accelerated pace | Pivot to M&A, dilutive acquisition | FCF deployment quality |
| **OT/IoT security TAM** | Early stage | TAM sizing >$15B, dedicated OT roadmap, Iran-crisis urgency | Afterthought, no dedicated resources | Growth vector validation |
| **AI security positioning** | Nascent | AI-native products, LLM threat detection, GenAI SecOps | Buzzword-only, no product substance | Competitive moat vs CRWD/PANW |
| **Lacework/FortiCNAPP traction** | CNAPP Leader (KuppingerCole), DSPM added Jan 2026 | Customer wins, ARR disclosure, Wiz competitive positioning | No customer adoption data, silent on cloud | Cloud security credibility |

### Scenario Impact on Thesis

| Outcome | FV Impact | Action |
|---------|-----------|--------|
| **BULL** (SASE ARR >$1B, next-gen ASIC details, FY27 guide >12% CAGR, strong AI/OT positioning) | FV $88 -> $95-100 | HOLD, raise FV. At current price $83.67 this creates MoS ~14-16% and E[CAGR] ~14-16%. ADD becomes reasonable. |
| **BASE** (incremental updates, confirms FY26 guide, SASE steady, no major surprises) | FV $88 unchanged | HOLD. E[CAGR] stays at ~9.7%. Lowest in portfolio. Monitor for rotation candidacy if pipeline matures. |
| **BEAR** (SASE deceleration signal, no ASIC roadmap, margin concerns, CVE dismissal) | FV $88 -> $75-80 | Review KC#2. If FV drops to $78, position is UNDERWATER and becomes rotation candidate. EXIT Protocol triggered. |

### FV Sensitivity Analysis

**If SASE growth >25% is announced:**
- SASE billings of $1.5B (est.) growing at 30%+ = $1.95B by FY2027, $2.54B by FY2028
- SASE at 30% of billings by FY2028 (vs ~24% today) accelerates services mix
- Higher services mix -> higher gross margins -> higher FCF/share
- Impact on OEY method: FCF grows from $2.2B to $2.8-3.0B by FY2028
- Owner Earnings Yield at $83.67 improves from ~2.6% to projected ~3.5%
- FV impact: +$7-10 (FV $95-98)
- E[CAGR] at $83.67 would improve from 9.7% to 13-15%

**If margins guided DOWN (e.g., GM <80% target):**
- GM decline from 81% to 79% = ~$136M lower gross profit per year
- If driven by SASE infrastructure costs (data centers, cloud capacity), this is TEMPORARY and EXPECTED during investment phase
- If driven by competitive pricing pressure (PANW bundling, hyperscaler competition), this is STRUCTURAL and concerning
- Need to distinguish: investment-phase margin compression (acceptable) vs loss-of-pricing-power compression (KC-approaching)
- FV impact if structural: -$5-8 (FV $80-83). At $83.67 this creates NEGATIVE MoS -> EXIT candidate
- FV impact if investment-phase: -$2-3 (FV $85-86). Manageable, margin recovery expected in 12-18 months

**Price sensitivity for E[CAGR] improvement:**
- At FV $88 (unchanged): price needs to drop to $75 for E[CAGR] ~14% (meaningful)
- At FV $95 (bull case): current price $83.67 gives E[CAGR] ~14.2% (attractive)
- At FV $100 (strong bull): current price $83.67 gives E[CAGR] ~15.9% (very attractive)
- At FV $80 (bear): current price $83.67 gives E[CAGR] ~7.4% (below threshold, rotation candidate)

### What I'm Looking For (Edge)

1. **SASE customer count + churn.** Market focuses on billings. If customer count is rising AND churn is <5%, the recurring base is stronger than market credits. This is the single most important disclosure for raising FV.
2. **ASIC cost advantage quantification.** If Fortinet reveals specific $/Gbps vs x86, the moat story becomes more concrete for re-rating. SP5 was launched in 2023 -- any SP6 timeline or specs would be material.
3. **Cross-sell data.** Products per customer trending up = platform stickiness = higher switching costs = justifies premium multiple.
4. **Security incident remediation.** How they address CVE-2025/2026 publicly matters. Transparency = trust = customer retention. Session catalog confirms "product security practices" is on the agenda -- this is encouraging.
5. **Iran/geopolitical cyber angle.** With active Iran war and state-sponsored cyber threats, any OT/critical infrastructure security announcements have immediate market relevance. Adobe, ExxonMobil customer stories could validate this vector.
6. **FortiCNAPP / Lacework traction.** Lacework was a fire-sale acquisition. If FortiCNAPP is gaining enterprise traction (KuppingerCole Leader recognition + SC Awards already secured), this validates the cloud security expansion thesis. DSPM added Jan 2026 shows active development.
7. **Post-quantum cryptography.** Conference agenda mentions FortiOS with post-quantum crypto. If substantive, this is a forward-looking moat reinforcement that few competitors are addressing.

### KC Mapping to Investor Day Signals

| Kill Condition | Investor Day Signal to Watch | How It Appears |
|----------------|------------------------------|----------------|
| KC#1 (Rev <8% 2Q) | FY2027+ revenue guide | If long-term guide implies <8% CAGR -> early warning |
| KC#2 (SASE ARR <15%) | Unified SASE ARR disclosure | If ARR growth disclosed <20% -> approaching KC |
| KC#5 (Xie <5% ownership) | Capital allocation commentary | Unlikely at investor day, but monitor any large sell-program announcements |
| KC#6 (2nd security incident) | CVE-2025/2026 response | How they frame past incidents. If dismissive/no remediation data -> risk elevated |
| KC#7 (Product rev -10% + SASE <15%) | Product roadmap + SASE trajectory | Combined weakness here is thesis-breaking |

**CRITICAL: If investor day reveals SASE deceleration (<25% growth) WITHOUT offsetting product strength, this approaches KC#2 territory. Trigger review-agent immediately.**

### Geopolitical Tailwind Context (Mar 7, 2026)

The Iran/Hormuz crisis (Operation Epic Fury, Day 7+) creates a structural cybersecurity demand tailwind:
- Iran's "Electronic Operations Room" (formed Feb 28) coordinating attacks on Western critical infrastructure
- CISA, UK NCSC, Canadian CCCS all issued Iranian cyber threat advisories
- OT/IoT security spending urgency is materially elevated
- FTNT is our portfolio's best crisis beneficiary (sector view confirms: FAVORABLE in crisis)
- If Accelerate 2026 addresses geopolitical cyber threats substantively (likely given customer base includes ExxonMobil, a direct Iran/Hormuz-exposed entity), this validates OT security growth vector
- However: macro risk-off (VIX 29.49, oil $91) compresses all tech multiples including FTNT. The demand tailwind may not translate to immediate stock price appreciation

### Pre-Event Checklist

- [x] Check FTNT price morning of Mar 10 ($84.20, +3.2% from cost basis $81.58)
- [x] Monitor live webcast at 3:00 PM PT via investor.fortinet.com
- [x] Post-event: update thesis Section 9 with actuals vs framework
- [x] If any KC approaches: NO KCs approached. All clear.
- [x] Specifically look for: ASIC ($1B+ invested, 60% unit share, 2x apps), SASE ARR (not disclosed — gap), products/customer (not disclosed — gap), FY27 guide (no specific raise — $9.2B rev by FY28 projected), CVE response (addressed in briefing, no churn data)
- [x] BULL/BEAR assessment: NEUTRAL/SLIGHT BULL — not enough for FV raise, nothing bearish

### Post-Event Assessment (2026-03-11)

**VERDICT: NEUTRAL with BULL leanings. FV $88 UNCHANGED.**

**What was disclosed:**
- FortiOS 8.0 launched: AI-driven security, next-gen SASE, quantum-safe protection. Substantive.
- FortiSOC launched: new cloud SecOps platform, consolidates tools. Real product substance.
- FortiASIC: >$1B invested, 60% unit share, 2x more apps than prior gen. Moat confirmed.
- SASE billings: +40% Q4, +24% FY, now 27% of total billings. ABOVE framework threshold.
- Business segments: Secure Networking +12%, Unified SASE +24%, AI-Driven SecOps +22%.
- TAM: >$300B addressable, <20% penetration. OT expanding >20%.
- Sovereign SASE, AI/quantum-safe positioned as greenfield opportunities.
- 75% enterprise digital sovereignty adoption expected by 2030.
- FY2028 projected: $9.2B revenue, $2.4B earnings (implies ~12% revenue CAGR from FY2025).

**What was NOT disclosed (gaps):**
- No specific SASE ARR figure (needed >$1B for BULL)
- No specific FY2027 revenue guide raise
- No next-gen ASIC (SP6) timeline
- No products/customer attach rate
- No CVE churn metrics
- No margin guidance update

**Why FV stays at $88:**
- FY2028 $9.2B revenue = ~12% CAGR from FY2025 $6.8B. Already in our Base case.
- SASE +40% Q4 is strong but was KNOWN from Q4 earnings (Feb 5). Not new information.
- FortiOS 8.0 and FortiSOC are encouraging for moat durability but don't change near-term financials.
- No data point forces FV upward. Need Q1 FY2026 actuals (~May 6) to validate trajectory.

**KCs status post-event:**
- KC#2 (SASE <15%): CLEAR, very clear — +40% Q4, +24% FY
- KC#6 (2nd security incident): MONITORING ELEVATED — CVE pattern escalating (11 new vulns Mar 10, active exploitation confirmed, MFA bypass CVE-2026-22572)
- KC#9 (3rd CISA KEV): MONITORING ELEVATED — CISA guidance issued on CVE-2026-24858. Need precise 12mo KEV count.

**CVE ESCALATION UPDATE (S151c28, 2026-03-13):**
On the SAME DAY as the investor briefing (Mar 10), 11 new vulnerabilities were disclosed including:
- CVE-2025-54820: HIGH severity RCE in FortiManager (remote unauthenticated)
- CVE-2026-22572: Authentication bypass that bypasses MFA entirely (FUNDAMENTAL security failure)
- Active exploitation confirmed: CVE-2025-59718, CVE-2025-59719, CVE-2026-24858
- CISA issued guidance on ongoing exploitation
This is NOT a one-off — it's a PATTERN. KC#6 and KC#9 both ELEVATED. FV $88 unchanged but conviction WEAKENING.
See: state/ftnt_accelerate_2026.md for full report.

**E[CAGR]: 9.4% at $84.40 vs FV $88.** Worst in portfolio. ROTATION CANDIDATE — first out when better candidate matures + capital available. Q1 earnings May 6 is next catalyst. If Q1 FAIL → EXIT Protocol triggered.

**Stock reaction: flat** ($81.62-$84.86 range). Market agrees — nothing surprising.

*Framework prepared: 2026-03-06 (S141). KC mapping added 2026-03-07 (S142). Pre-event review completed 2026-03-07 (S143). QS updated 87 (S144c3). Post-event assessment: 2026-03-11 (S151). CVE escalation update: 2026-03-13 (S151c28).*

---

*Position opened: 2026-02-26. 5.98 shares @ $79.12 (EUR 400). Added 6.94 shares @ ~$83.70. Trimmed 2.35 shares @ $83.67 (S143c8 T15 sizing rebalance). Current: 10.57 shares @ avg $81.58.*
*R1->R2->R3->R4 pipeline completed S123. 5th E[CAGR]-framework market buy.*

---

## Section 10: Q1 CY2026 Earnings Framework (~Late April 2026)

*Built: 2026-03-08 (S149). Updated post-Investor Day 2026-03-11 (S151): Investor Day was NEUTRAL — no guidance change. FV $88 unchanged. Framework baseline unchanged. Q1 is now the deciding data point for position survival.*

### Context: Why Q1 Matters

FTNT is 2nd worst E[CAGR] (9.7%) in portfolio. Investor Day Mon Mar 10 will either rescue or condemn the position. But Q1 earnings (~late April) is the FIRST quarterly data point of FY2026. It tests whether FY2026 guidance ($7.5-7.7B revenue) is on track, whether the firewall refresh cycle has legs, and whether SASE can sustain +40% billings growth.

**FY2025 actuals (baseline):** Revenue $6.8B (+14%), Billings $7.55B (+16%), Q4 billings $2.37B (+18% = acceleration), Services rev $4.58B (+13%), Product rev $2.22B (+16%), GM 81.3%, OP margin 35.5% non-GAAP, FCF $2.21B (32.5% margin), Deferred rev $7.1B (+15%), EPS $2.37 non-GAAP.

**FY2026 guidance:** Revenue $7.5-7.7B (+10-13%), Billings $8.0-8.3B (+6-10%), OP margin 33-34% non-GAAP, EPS $2.43-2.53.

### Key Metrics for Q1

| Metric | Q1 FY2025 Actual | Q1 FY2026 Needed | PASS | FAIL |
|--------|-----------------|-----------------|------|------|
| **Revenue** | $1.54B | $1.72-1.77B | >=12% YoY | <10% YoY |
| **Billings** | $1.58B | $1.74-1.82B | >=10% YoY | <6% YoY (guide floor) |
| **Product revenue** | $454M | $500M+ | >=10% YoY (refresh cycle) | <5% (refresh fading) |
| **Services revenue** | $1.09B | $1.22B+ | >=12% YoY | <10% (decel) |
| **Unified SASE billings** | ~$300M est | ~$400M+ | >=30% YoY | <20% (KC#2 risk) |
| **Gross margin** | 80.2% | >=80% | Stable/expanding | <78% (structural) |
| **Operating margin** | 33.1% non-GAAP | >=32% | On guide | <30% (investment overshoot) |
| **FCF** | $818M | >=35% margin | Strong conversion | <25% (concern) |
| **Deferred revenue** | $6.43B | $7.2B+ | >=12% growth | <8% (KC#8 risk) |
| **EPS non-GAAP** | $0.57 | $0.58-0.62 | On guide | <$0.55 |

### Decision Tree

| Scenario | Criteria | Action |
|----------|----------|--------|
| **PASS** | Revenue >=12%, billings >=10%, SASE >=30%, GM >=80%, guide maintained/raised | **HOLD.** If Investor Day was BULL, this confirms FV $95-100. If BASE, confirms $88. |
| **CONDITIONAL** | Revenue 10-12%, billings 6-10% (guide floor), SASE 20-30%, GM 78-80% | **HOLD but review.** Monitor Q2 closely. If E[CAGR] still <12% and pipeline has rotation candidate → prepare EXIT. |
| **FAIL** | Revenue <10% OR billings <6% OR SASE <20% OR GM <78% OR guide cut | **EXIT.** KC#1 approaching (rev <8% 2Q), KC#2 risk (SASE <15%), or KC#8 (deferred rev <8%). Run EXIT Protocol 6 gates. FTNT → GDDY rotation activated. |

### KC Mapping to Q1 Data Points

| KC | Q1 Trigger Signal | Pre-Trigger Alert |
|----|-------------------|-------------------|
| KC#1 (rev <8% 2Q) | Q1 rev growth <10% = first flag | Two weak Qs = TRIGGERED |
| KC#2 (SASE <15%) | Unified SASE billings growth disclosed <20% | If <25% → APPROACHING |
| KC#3 (FCF <25%) | FCF margin <25% in Q1 | One Q can be seasonal; 2Q = KC |
| KC#6 (2nd incident) | Any new CVE in Q1 | Check CISA KEV post-earnings |
| KC#7 (product -10% + SASE <15%) | Product rev negative + SASE <20% | Combined weakness = thesis-breaking |
| KC#8 (deferred rev <8% 2Q) | Deferred rev growth <10% | Was 15% FY2025, decel to <10% = concern |
| KC#9 (3rd CISA KEV/12mo) | New CISA KEV entry in Q1 | Check post-event |

### FV Sensitivity to Q1 Outcome

| Q1 Outcome | Impact on FV | E[CAGR] at ~$84 |
|------------|-------------|-----------------|
| Strong PASS + guide raise | FV $88 → $95-100 | 14-16% (justified HOLD) |
| PASS + guide maintained | FV $88 unchanged | 9.7% (rotation candidate) |
| CONDITIONAL + guide maintained | FV $88 → $83-85 | 7-9% (EXIT candidate) |
| FAIL + guide cut | FV $88 → $70-75 | Negative (EXIT immediately) |

### What Changes if Investor Day Already Happened

- **If Investor Day BULL:** Q1 PASS = double confirmation. FV already at $95-100. Q1 validates. HOLD firm.
- **If Investor Day NEUTRAL:** Q1 becomes the deciding quarter. PASS = HOLD at $88. CONDITIONAL = last chance.
- **If Investor Day BEAR:** Position may already be sold (→ GDDY rotation). Framework is MOOT unless still holding.
- **If FTNT SOLD before Q1:** Monitor Q1 for re-entry signal. If Q1 PASS + price <$75, consider SO.

### Refresh Cycle Risk

The FY2025 product revenue surge (+16%) was partly driven by FortiGate firewall refresh cycle (enterprises replacing aging units). Q1 tests whether this cycle has legs or was a one-quarter phenomenon. Key signals:
- Product revenue growth >=10% = cycle continuing
- Product revenue growth <5% = cycle exhausting (FY2026 product rev at risk)
- FortiGate unit shipments (if disclosed) vs ASP growth = volume vs pricing decomposition

### Competitive Context

Monitor earnings/commentary from:
- **PANW** (Q2 FY2026 ~Feb, already reported) — platformization momentum, NGS ARR
- **CRWD** (Q4 FY2026 ~Mar, if available by Q1 FTNT) — endpoint market health
- **ZS** (Q2 FY2026 ~Mar) — SASE/SSE pure-play trajectory

If PANW shows accelerating platformization while FTNT decelerates → moat under siege.

*Framework prepared: 2026-03-08 (S149c29). Independent of Investor Day outcome. Update with Investor Day baseline adjustments post-Mar 10.*
